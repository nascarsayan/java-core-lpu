...
CREATE TABLE utV (
                     ...
                     V_COLOR TEXT NOT NULL CHECK (LENGTH(V_COLOR) = 1)
);

CREATE TABLE utB (
                     B_DATETIME TEXT NOT NULL,
                     B_Q_ID INTEGER NOT NULL,
                     B_V_ID INTEGER NOT NULL,
                     B_VOL INTEGER NOT NULL CHECK (B_VOL >= 0 AND B_VOL <= 255),
                     PRIMARY KEY (B_DATETIME, B_Q_ID, B_V_ID),
                     FOREIGN KEY (B_Q_ID) REFERENCES utQ(Q_ID),
                     FOREIGN KEY (B_V_ID) REFERENCES utV(V_ID)
);
